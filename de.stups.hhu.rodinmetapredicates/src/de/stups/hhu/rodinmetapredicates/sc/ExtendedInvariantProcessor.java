...
public class ExtendedInvariantProcessor extends Processor {
	...
	public void process(IRodinElement element, IInternalElement target, ISCStateRepository repository,
		...
		ExtendedInvariant[] extendedInvariants = machineRoot.getChildrenOfType(ExtendedInvariant.ELEMENT_TYPE);
		...
		for (ExtendedInvariant ei : extendedInvariants) {
			IParseResult parsed = ff.parsePredicate(ei.getPredicateString(), null);
			if (parsed.getProblems().isEmpty()) {
				...
				if (rr.rewriteFailed()) {
					ei.createProblemMarker(new ReplacementFailedMarker(ei.getPredicateString()));
					return;
				}

				ISCInvariant newInvariant = scMachineRoot.createChild(ISCInvariant.ELEMENT_TYPE, null, monitor);
				newInvariant.setLabel(getNextLabel(), monitor);
				newInvariant.setPredicate(rewritten, monitor);
				newInvariant.setPredicateString(rewritten.toStringFullyParenthesized(), monitor);
				newInvariant.setSource(ei, monitor);
				newInvariant.setTheorem(ei.isTheorem(), monitor);
			} else {
				ei.createProblemMarker(new MetaPredicateNotParsedMarker(ei.getPredicateString()));
			}
		}

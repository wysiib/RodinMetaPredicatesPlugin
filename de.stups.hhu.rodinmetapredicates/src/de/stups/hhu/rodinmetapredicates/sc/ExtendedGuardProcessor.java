...
public class ExtendedGuardProcessor extends Processor {
	...
	public void process(IRodinElement element, IInternalElement target, ISCStateRepository repository,
		...
		FormulaFactory ff = scMachineRoot.getFormulaFactory().withExtensions(getFormulaExtensions());
		...
		for (IEvent evt : events) {
			...
			ExtendedGuard[] eGuards = evt.getChildrenOfType(ExtendedGuard.ELEMENT_TYPE);
			...
			if (scEvt != null && eGuards.length > 0) {
				for (ExtendedGuard eGuard : eGuards) {
					IParseResult parsed = ff.parsePredicate(eGuard.getPredicateString(), null);
					if (parsed.getProblems().isEmpty()) {
						...
						if (rr.rewriteFailed()) {
							eGuard.createProblemMarker(new ReplacementFailedMarker(eGuard.getPredicateString()));
							return;
						}
						ISCGuard newGuard = scEvt.createChild(ISCGuard.ELEMENT_TYPE, null, monitor);
						newGuard.setLabel(getNextLabel(), monitor);
						newGuard.setPredicate(rewritten, monitor);
						newGuard.setPredicateString(rewritten.toStringFullyParenthesized(), monitor);
						newGuard.setSource(eGuard, monitor);
						newGuard.setTheorem(eGuard.isTheorem(), monitor);
					} else {
						eGuard.createProblemMarker(new MetaPredicateNotParsedMarker(eGuard.getPredicateString()));
					}
				}
			}
		}
	...
	private ISCEvent getSCEvent(ISCMachineRoot root, String label) throws RodinDBException {
